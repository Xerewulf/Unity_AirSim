...
public class AirplaneController : MonoBehaviour
{
    ...
    Rotator propeller;// here because propelars speed
    ...
    private void Update()
    {
        ...
        {
            SceneManager.LoadScene(0);//reset the sceen
        }

        if (Input.GetKey(KeyCode.Space))
        {
            SetThrust(thrustPercent + thrustControlSensitivity);//giving thrust to plane
        }
        propeller.speed = thrustPercent * 1500f;

        if (Input.GetKeyDown(KeyCode.LeftShift))//making trush to revers it slow down to plane
        {
