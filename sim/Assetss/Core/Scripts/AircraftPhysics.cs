...
public class AircraftPhysics : MonoBehaviour
{
    ...
    public void SetControlSurfecesAngles(float pitch, float roll, float yaw, float flap)//here for do pitch, roll, yaw in controlSurface 
    {
    ...
    private BiVector3 CalculateAerodynamicForces(Vector3 velocity, Vector3 angularVelocity, Vector3 wind, float airDensity, Vector3 centerOfMass)//here for add up for all airodynamic forces
    {
        ...
        }                                                       // calculate what it needs for rigidbody
    ...
    public void Brake(bool isBraking) //increases drag on wheels  -- instead of wheels ı use spherecollider with splippery , with 0 friction
    {
        //add drag on wheels
        ...
        if (isBraking)
        {
            friction = 0.2f;// when hit the break it will increase friction
        }
        else
        {
            friction = 0f;
